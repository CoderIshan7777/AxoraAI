...
const BlogTitles = () => {
   ...
   const blogCategories = ['General','Technology','Business', 'Health',
    ...
    const onSubmitHandler = async (e) =>{
      ...
      try{
        ...
        const {data} = await axios.post('http://localhost:3000/api/ai/generate-blog-title', {prompt, length: 100}, {headers: {Authorization: `Bearer ${await getToken()}`}})
  ...
  return (
     <div className="bg-[#E8DFCA] h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
      <form onSubmit={onSubmitHandler} className="w-full max-w-lg p-4 bg-[#F5EFE6] rounded-lg border border-gray-200">
        {/* Header */}
            ...
            <span
              ...
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer transition ${
                selectedCategory === item
                  ...
                  : 'text-gray-500 border-gray-300 hover:border-purple-400 hover:text-purple-600'
              }`}
            >
              {item}
            </span>
      ...
      <div className='w-full max-w-lg p-4 bg-[#F5EFE6] rounded-lg flex flex-col border
